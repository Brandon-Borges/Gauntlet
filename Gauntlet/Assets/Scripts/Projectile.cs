...
public class Projectile : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.transform.tag != playerTag)
        {
            ...
            {
                player.GetComponent<Warrior>().isFiring = false;
            }
            else if (playerTag == "Valkyrie")
            {
                player.GetComponent<Valkyrie>().isFiring = false;
            }
            else if (playerTag == "Wizard")
            {
                player.GetComponent<Wizard>().isFiring = false;
            }
            else if (playerTag == "Elf")
            {
                player.GetComponent<Elf>().isFiring = false;
            }
